...
const main = async rengar => {
  const championIdMap = await rengar.DDragon.Champion.listDataByIdWithParentAsId()
  const { id, accountId } = await rengar.Summoner.by.name('Jeongsik Oh')
  const { matches } = await rengar.Matchlist.by
    .accountID(accountId)
    .query({ queue: 420 })
  const gameIds = matches.slice(0, 10).map(({ gameId }) => gameId)
  const matchDtos = await Promise.all(gameIds.map(rengar.Match.get))
