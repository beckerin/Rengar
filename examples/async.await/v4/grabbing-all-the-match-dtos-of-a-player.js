...
// getFN is required because of this file structure where `rengar` isn't included.
...
const main = async rengar => {
  ...
  const { accountId: accountID } = await rengar.Summoner.by.name(
    '9 5 mcdonalds',
  )
  ...
  const matches = await Promise.all(matchIDs.map(rengar.Match.get))
  console.log(matches)
