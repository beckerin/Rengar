...
function Request(
    ...
    if (this.apiURLPrefix == undefined) this.apiURLPrefix = config.apiURLPrefix
    this.config = config
...
Request.prototype.callback = function (cb) {
    ...
    const serverPrefix = isMatch ? RegionHelper.asContinent(region) : RegionHelper.asPlatformID(region)
...
Request.prototype.execute = async function (url, body, token, retriesLeft, cb) {
    ...
    const fn = async (err, data) => {
        const debugURL = `${url}${debugOptions.showKey ? URLHelper.getAPIKey(url, token) : ''
            }`
        try {
            if (data) {
