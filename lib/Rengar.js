...
import { DEFAULT_RENGAR_CONFIG, RENGAR_CONFIG_STRUCT } from './RengarConfig'

class Rengar {
  constructor(
    key = process.env.RIOT_LOL_API_KEY,
    config = DEFAULT_RENGAR_CONFIG,
  ) {
    if (!ParameterHelper.isKeyValid(key)) {
      throw new Error(
        'Failed to initialize Rengar! API key is not a non-empty string.',
      )
    }
    ...
    this.config = RENGAR_CONFIG_STRUCT(
      merge(merge({}, DEFAULT_RENGAR_CONFIG), { key, ...config }),
    )
...
const init = key => config => new Rengar(key, config)
